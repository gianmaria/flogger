...
public:
    ...
    static std::string getCurrentDate()
    {
        ...
        oss << std::setfill('0') << std::setw(2) << local_tm->tm_mday << "/" // day
            << std::setw(2) << (local_tm->tm_mon + 1) << "/"                 // month (Jan is 0)
            << (local_tm->tm_year + 1900);                                   // year (years since 1900)
    ...
    static std::string get_TID()
    {
        ...
        std::ostringstream oss;
        oss << std::hex << std::setw(8) << std::setfill('0') << std::uppercase << this_id;
...
protected:
    Flogger(std::string_view log_file_path) : log_file_path(log_file_path)
    {
...
private:
    ...
    void flush_Buffer()
    {
        if (not buffer.str().empty() && out_file.is_open())
        {
            out_file << buffer.str();
...
#define Flog(fmt, ...)                              \
    default_flog.log(Flogger::time | Flogger::file, \
                     Flogger::get_Current_Time(),   \
                     Flogger::get_TID(),            \
                     __FILE__,                      \
                     __LINE__,                      \
                     __func__,                      \
...
#define Ftrace        \
    default_flog.log( \
      Flogger::file, Flogger::get_Current_Time(), Flogger::get_TID(), __FILE__, __LINE__, __func__, "{}", "*trace*")
